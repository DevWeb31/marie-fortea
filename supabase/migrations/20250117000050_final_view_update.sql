-- Migration finale pour s'assurer que la vue utilise la bonne colonne
-- Date: 2025-01-17
-- Objectif: Recréer la vue pour utiliser duration_hours (pas duration_hours_fixed)
...
-- 2. Recréer la vue pour utiliser duration_hours (la colonne principale)
...
SELECT 
    ...
    br.start_time,
    br.end_time,
    br.duration_hours,  -- Utiliser la colonne principale (maintenant corrigée)
    br.children_count,
    br.children_details,
    br.children_ages,
...
-- 3. Vérifier l'état final de la vue
SELECT 
    'État final de la vue' as status,
    COUNT(*) as total_records,
    COUNT(CASE WHEN duration_hours < 0 THEN 1 END) as negative_durations,
...
-- 4. Afficher les exemples finaux
SELECT 
    'Exemples finaux de la vue' as examples,
    id,
    parent_name,
    start_time,
    end_time,
...
-- 5. Test spécifique pour les cas problématiques
SELECT 
    'Test des cas problématiques' as test,
    id,
    parent_name,
    start_time,
    end_time,
    duration_hours,
    CASE 
        WHEN start_time > end_time THEN 'Passe minuit'
        ELSE 'Même jour'
    END as type_garde,
    CASE 
        WHEN duration_hours < 0 THEN '❌ NÉGATIF'
    ...
    END as status
FROM booking_requests_with_status 
WHERE start_time > end_time  -- Cas qui passent minuit
ORDER BY created_at DESC 
LIMIT 3;

-- Message final
SELECT 'Vue finale mise à jour - duration_hours devrait maintenant être correcte' as final_status;
